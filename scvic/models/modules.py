...
from scvi.models.modules import reparameterize_gaussian
...
class FCLayersList(nn.Module):
    ...
    def __init__(
        ...
        n_out: int,
        n_hidden: List[int],
        n_cat_list: Iterable[int] = None,  # 可以每列是个类别属性
        ...
        super().__init__()
        layers_dim = [n_in] + n_hidden + [n_out]  # n_out算在n_layers里

        if n_cat_list is not None:
            # n_cat = 1 will be ignored
            self.n_cat_list = [n_cat if n_cat > 1 else 0 for n_cat in n_cat_list]  # 将<=1强制变成0
        else:
            self.n_cat_list = []

        self.fc_layers = nn.Sequential(
            collections.OrderedDict(  # 结合for循环快速写入有序字典
                [
                    (
                        "Layer {}".format(i),
                        nn.Sequential(
                            nn.Linear(n_in + sum(self.n_cat_list), n_out, bias=bias),  # 输入大小是输入特征的大小加上有效的类别属性的预留大小
    ...
    def forward(self, x: torch.Tensor, *cat_list: int, instance_id: int = 0):
        ...
        assert len(self.n_cat_list) <= len(
            cat_list  # 为有效的类别属性的预留位置 forward传递的时候一定要有值 多的部分会被截掉 但是不能少
        ), "nb. categorical args provided doesn't match init. params."
        for n_cat, cat in zip(self.n_cat_list, cat_list):
            assert not (
                n_cat and cat is None
            ), "cat not provided while n_cat != 0 in init. params."  # 不能存在定义了n_cat 但是实例没有cat传入的情况
            if n_cat > 1:  # n_cat = 1 will be ignored - no additional information
                if cat.size(1) != n_cat:
                    one_hot_cat = one_hot(cat, n_cat)
                else:
                    one_hot_cat = cat  # cat has already been one_hot encoded
                    # 如果cat列的维度等于n_cat 代表输入就已经是one_hot了
                one_hot_cat_list += [one_hot_cat]
        for layers in self.fc_layers:
            for layer in layers:
                if layer is not None:
                    if isinstance(layer, nn.BatchNorm1d):
                        if x.dim() == 3:  # x.dim为什么可以等于3
                            x = torch.cat(
