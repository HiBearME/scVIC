...
save_dir="result_imbalanced_dropout4"
...
for i_ratio in range(5):
    for i_duplicate in range(10):
        ...
        ARI_DESC[i_ratio, i_duplicate] = np.around(adjusted_rand_score(gene_dataset.labels.squeeze(), adata.obs["desc_1.0"]), 5)
        NMI_DESC[i_ratio, i_duplicate] = np.around(normalized_mutual_info_score(gene_dataset.labels.squeeze(), adata.obs["desc_1.0"], average_method='arithmetic'), 5)
        ACC_DESC[i_ratio, i_duplicate] = np.around(cluster_acc(gene_dataset.labels.squeeze(), adata.obs["desc_1.0"]), 5)
        BatchMixing_DESC[i_ratio, i_duplicate] = np.around(entropy_batch_mixing(adata.obsm["X_Embeded_z1.0"], gene_dataset.batch_indices, gene_dataset.n_batches), 5)
print("ARI_DESC: ")
print(ARI_DESC)
np.save("saved/ARI_DESC_imbalanced_dropout4.npy", ARI_DESC)

print("NMI_DESC: ")
print(NMI_DESC)
np.save("saved/NMI_DESC_imbalanced_dropout4.npy", NMI_DESC)

print("ACC_DESC: ")
print(ACC_DESC)
np.save("saved/ACC_DESC_imbalanced_dropout4.npy", ACC_DESC)

print("BatchMixing_DESC: ")
print(BatchMixing_DESC)
np.save("saved/BatchMixing_DESC_imbalanced_dropout4.npy", BatchMixing_DESC)                                                    
