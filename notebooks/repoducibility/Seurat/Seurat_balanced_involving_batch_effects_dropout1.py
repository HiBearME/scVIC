...
for i_cluster in range(5):
    for i_duplicate in range(10):
        ...
        "/Users/Healthy/Desktop/final/R/generate simulated datasets/data/Balanced Datasets Involving Batch Effects/" + str(i_cluster + 3) + "/dropout" + str(i_dropout + 1) +"/batches_duplicate" + str(i_duplicate + 1) + ".csv",
                    index_col=0).values.squeeze()
        pca = pd.read_csv(
        "./intermediate_saving/pca_for_Seurat/balanced_involving_batch_effects/" + str(i_cluster + 3) + "/dropout" + str(i_dropout + 1) +"/pca_duplicate" + str(i_duplicate + 1) + ".csv",
                    sep = ";", decimal=',', index_col=0).values
        ...
        ARI_Seurat_Louvain[i_cluster, i_duplicate] = np.around(adjusted_rand_score(labels, data.obs["louvain"]), 5)
        NMI_Seurat_Louvain[i_cluster, i_duplicate] = np.around(normalized_mutual_info_score(labels, data.obs["louvain"], average_method='arithmetic'), 5)
        ACC_Seurat_Louvain[i_cluster, i_duplicate] = np.around(cluster_acc(labels, data.obs["louvain"].astype(int)), 5)                                                                
        BatchMixing_Seurat_Louvain[i_cluster, i_duplicate] = np.around(entropy_batch_mixing(pca, np.array(batch_indices), n_batches), 5)

print("ARI_Seurat_Louvain: ")
print(ARI_Seurat_Louvain)
np.save("saved/ARI_Seurat_Louvain_balanced_involving_batch_effects_dropout1.npy", ARI_Seurat_Louvain)

print("NMI_Seurat_Louvain: ")
print(NMI_Seurat_Louvain)
np.save("saved/NMI_Seurat_Louvain_balanced_involving_batch_effects_dropout1.npy", NMI_Seurat_Louvain)

print("ACC_Seurat_Louvain: ")
print(ACC_Seurat_Louvain)
np.save("saved/ACC_Seurat_Louvain_balanced_involving_batch_effects_dropout1.npy", ACC_Seurat_Louvain)     

print("BatchMixing_Seurat_Louvain: ")
print(BatchMixing_Seurat_Louvain)
np.save("saved/BatchMixing_Seurat_Louvain_balanced_involving_batch_effects_dropout1.npy", BatchMixing_Seurat_Louvain)