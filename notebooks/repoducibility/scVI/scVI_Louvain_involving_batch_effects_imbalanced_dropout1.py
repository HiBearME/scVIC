...
for i_ratio in range(5):
    for i_duplicate in range(10):
        ...
        batches = pd.read_csv(
        "/Users/Healthy/Desktop/final/R/generate simulated datasets/data/Imbalanced Datasets Involving Batch Effects/ratio" + str(i_ratio + 1) + "/dropout" + str(i_dropout + 1) +"/batches_duplicate" + str(i_duplicate + 1) + ".csv", index_col=0).values.squeeze()
        ...
        ARI_scVI_Louvain[i_ratio, i_duplicate] = np.around(adjusted_rand_score(gene_dataset.labels.squeeze(), data.obs["louvain"]), 5)
        NMI_scVI_Louvain[i_ratio, i_duplicate] = np.around(normalized_mutual_info_score(gene_dataset.labels.squeeze(), data.obs["louvain"], average_method='arithmetic'), 5)
        ACC_scVI_Louvain[i_ratio, i_duplicate] = np.around(cluster_acc(gene_dataset.labels.squeeze(), data.obs["louvain"].astype(int)), 5)
        BatchMixing_scVI_Louvain[i_ratio, i_duplicate] = np.around(entropy_batch_mixing(latent, gene_dataset.batch_indices, gene_dataset.n_batches), 5)
        
print("ARI_scVI_Louvain: ")
print(ARI_scVI_Louvain)
np.save("saved/ARI_scVI_Louvain_imbalanced_involving_batch_effects_dropout1.npy", ARI_scVI_Louvain)

print("NMI_scVI_Louvain: ")
print(NMI_scVI_Louvain)
np.save("saved/NMI_scVI_Louvain_imbalanced_involving_batch_effects_dropout1.npy", NMI_scVI_Louvain)

print("ACC_scVI_Louvain: ")
print(ACC_scVI_Louvain)
np.save("saved/ACC_scVI_Louvain_imbalanced_involving_batch_effects_dropout1.npy", ACC_scVI_Louvain)

print("BatchMixing_scVI_Louvain: ")
print(BatchMixing_scVI_Louvain)
np.save("saved/BatchMixing_scVI_Louvain_imbalanced_involving_batch_effects_dropout1.npy", BatchMixing_scVI_Louvain)
