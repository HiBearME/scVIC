...
seed = 3 # when seed = 1,2 solution could not be found for scVIC_Louvian, use seed = 2 instead.
...
for i_ratio in range(5):
    for i_duplicate in range(10):
        ...
        gene_dataset.populate_from_data(X=counts, labels=labels, cell_types=cell_types)
        gene_dataset.keep_highly_variable_genes_by_seurat(new_n_genes=n_gene_highly_variable)
        n_labels = gene_dataset.n_labels
    
        cvae = CVAE(gene_dataset.nb_genes, n_labels=n_labels, n_latent=10)
        ctrainer = CTrainer(
            cvae,
            gene_dataset,
            train_size=1.0,
...
print("ARI_scVIC: ")
print(ARI_scVIC)
np.save("saved/ARI_scVIC_imbalanced_dropout1.npy", ARI_scVIC)

print("NMI_scVIC: ")
print(NMI_scVIC)
np.save("saved/NMI_scVIC_imbalanced_dropout1.npy", NMI_scVIC)

print("ACC_scVIC: ")
print(ACC_scVIC)
np.save("saved/ACC_scVIC_imbalanced_dropout1.npy", ACC_scVIC)

print("ARI_scVIC_Louvain: ")
print(ARI_scVIC_Louvain)
np.save("saved/ARI_scVIC_Louvain_imbalanced_dropout1.npy", ARI_scVIC_Louvain)

print("NMI_scVIC_Louvain: ")
print(NMI_scVIC_Louvain)
np.save("saved/NMI_scVIC_Louvain_imbalanced_dropout1.npy", NMI_scVIC_Louvain)

print("ACC_scVIC_Louvain: ")
print(ACC_scVIC_Louvain)
np.save("saved/ACC_scVIC_Louvain_imbalanced_dropout1.npy", ACC_scVIC_Louvain)            