...
for i_ratio in range(5):
    for i_duplicate in range(10):
        ...
        counts = pd.read_csv(
        "/Users/Healthy/Desktop/final/R/generate simulated datasets/data/Imbalanced Datasets Involving Batch Effects/ratio" + str(i_ratio + 1) + "/dropout" + str(i_dropout + 1) +"/counts_duplicate" + str(i_duplicate + 1) + ".csv",
        ...
        ARI_scVIC[i_ratio, i_duplicate] = np.around(adjusted_rand_score(gene_dataset.labels.squeeze(), labels_pred), 5)
        NMI_scVIC[i_ratio, i_duplicate] = np.around(normalized_mutual_info_score(gene_dataset.labels.squeeze(), labels_pred, average_method='arithmetic'), 5)
        ACC_scVIC[i_ratio, i_duplicate] = np.around(cluster_acc(gene_dataset.labels.squeeze(), labels_pred), 5)
        BatchMixing_scVIC[i_ratio, i_duplicate] = np.around(entropy_batch_mixing(latent, gene_dataset.batch_indices, gene_dataset.n_batches), 5)                                                 
        
        data = getNClusters(latent, n_cluster=n_labels)
        ARI_scVIC_Louvain[i_ratio, i_duplicate] = np.around(adjusted_rand_score(gene_dataset.labels.squeeze(), data.obs["louvain"]), 5)
        NMI_scVIC_Louvain[i_ratio, i_duplicate] = np.around(normalized_mutual_info_score(gene_dataset.labels.squeeze(), data.obs["louvain"], average_method='arithmetic'), 5)
        ACC_scVIC_Louvain[i_ratio, i_duplicate] = np.around(cluster_acc(gene_dataset.labels.squeeze(), data.obs["louvain"].astype(int)), 5)
        BatchMixing_scVIC_Louvain[i_ratio, i_duplicate] = np.around(entropy_batch_mixing(latent, gene_dataset.batch_indices, gene_dataset.n_batches), 5)
                                                                 
print("ARI_scVIC: ")
print(ARI_scVIC)
np.save("saved/ARI_scVIC_imbalanced_involving_batch_effects_dropout4.npy", ARI_scVIC)

print("NMI_scVIC: ")
print(NMI_scVIC)
np.save("saved/NMI_scVIC_imbalanced_involving_batch_effects_dropout4.npy", NMI_scVIC)

print("ACC_scVIC: ")
print(ACC_scVIC)
np.save("saved/ACC_scVIC_imbalanced_involving_batch_effects_dropout4.npy", ACC_scVIC)

print("BatchMixing_scVIC: ")
print(BatchMixing_scVIC)
np.save("saved/BatchMixing_scVIC_imbalanced_involving_batch_effects_dropout4.npy", BatchMixing_scVIC)

print("ARI_scVIC_Louvain: ")
print(ARI_scVIC_Louvain)
np.save("saved/ARI_scVIC_Louvain_imbalanced_involving_batch_effects_dropout4.npy", ARI_scVIC_Louvain)

print("NMI_scVIC_Louvain: ")
print(NMI_scVIC_Louvain)
np.save("saved/NMI_scVIC_Louvain_imbalanced_involving_batch_effects_dropout4.npy", NMI_scVIC_Louvain)

print("ACC_scVIC_Louvain: ")
print(ACC_scVIC_Louvain)
np.save("saved/ACC_scVIC_Louvain_imbalanced_involving_batch_effects_dropout4.npy", ACC_scVIC_Louvain)      

print("BatchMixing_scVIC_Louvain: ")
print(BatchMixing_scVIC_Louvain)
np.save("saved/BatchMixing_scVIC_Louvain_imbalanced_involving_batch_effects_dropout4.npy", BatchMixing_scVIC_Louvain)